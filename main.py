...
def get_yt_dlp_options(output_path: str) -> dict:
    ...
    # Try to use cookies from browser first (easiest method for stories)
    # This will automatically extract cookies from your logged-in browser
    if COOKIES_FILE.exists():
        ...
        try:
            ...
            try:
                ...
                logger.info("Attempting to use cookies from Firefox browser")
            except:
                try:
                    options['cookiesfrombrowser'] = ('safari',)
                    logger.info("Attempting to use cookies from Safari browser")
                except:
                    logger.info("No cookies available. Stories may not be accessible.")
